...
| --- | --- | --- | --- |
| 4.7.0-dev test binary, compared with upstream v4.7.0 | `github.com/containers/storage v1.50.2` | `SupportsShifting()` has no UID/GID map arguments. If an overlay mount program is configured, it reports shifting support without checking whether the active maps are contiguous. | No `_CONTAINERS_FORCE_SHIFTING` string was present. The v5.8.2-style patch does not apply verbatim. |
| Site 5.3.2, compared with upstream v5.3.2 | `github.com/containers/storage v1.56.1` | Same older shape: `SupportsShifting()` has no UID/GID map arguments and no contiguous-map check in this function. | No `_CONTAINERS_FORCE_SHIFTING` string was present. The v5.8.2-style patch does not apply verbatim. |
